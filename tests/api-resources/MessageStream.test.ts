...
`);
  ...
  it('keeps the accumulated stop_* when a later message_delta sends them as null', async () => {
    // Same convention as the `container` guard in the same block, which the
    // "keeps accumulated container" test exercises with an explicit `container: null`:
    // null means "not applicable here", so it must not clobber what we already have.
    const { fetch, handleStreamEvents } = mockFetch();
    const anthropic = new Anthropic({ apiKey: '...', fetch });
