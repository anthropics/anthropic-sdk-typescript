...
describe('resource memoryVersions', () => {
  test('retrieve: only required params', async () => {
    const responsePromise = client.beta.memoryStores.memoryVersions.retrieve('memory_version_id', { memory_store_id: 'memory_store_id' });
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.beta.memoryStores.memoryVersions.list('memory_store_id', {
  ...
  test('redact: only required params', async () => {
    const responsePromise = client.beta.memoryStores.memoryVersions.redact('memory_version_id', { memory_store_id: 'memory_store_id' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('redact: required and optional params', async () => {
    const response = await client.beta.memoryStores.memoryVersions.redact('memory_version_id', { memory_store_id: 'memory_store_id', betas: ['message-batches-2024-09-24'] });
  });
});
