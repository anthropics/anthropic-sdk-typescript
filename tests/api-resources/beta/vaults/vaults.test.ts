...
describe('resource vaults', () => {
  ...
  test('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.beta.vaults.retrieve('vlt_011CZkZDLs7fYzm1hXNPeRjv', { betas: ['message-batches-2024-09-24'] }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.beta.vaults.list({
  ...
  test('delete: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.beta.vaults.delete('vlt_011CZkZDLs7fYzm1hXNPeRjv', { betas: ['message-batches-2024-09-24'] }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Anthropic.NotFoundError);
  });
  ...
  test('archive: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.beta.vaults.archive('vlt_011CZkZDLs7fYzm1hXNPeRjv', { betas: ['message-batches-2024-09-24'] }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Anthropic.NotFoundError);
  });
});
