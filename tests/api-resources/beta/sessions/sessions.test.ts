...
describe('resource sessions', () => {
  test('create: only required params', async () => {
    const responsePromise = client.beta.sessions.create({ agent: 'agent_011CZkYpogX7uDKUyvBTophP', environment_id: 'env_011CZkZ9X2dpNyB7HsEFoRfW' });
  ...
  test('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.beta.sessions.retrieve('sesn_011CZkZAtmR3yMPDzynEDxu7', { betas: ['message-batches-2024-09-24'] }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.beta.sessions.list({
    agent_id: 'agent_id',
    agent_version: 0,
  ...
  test('delete: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.beta.sessions.delete('sesn_011CZkZAtmR3yMPDzynEDxu7', { betas: ['message-batches-2024-09-24'] }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Anthropic.NotFoundError);
  });
  ...
  test('archive: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.beta.sessions.archive('sesn_011CZkZAtmR3yMPDzynEDxu7', { betas: ['message-batches-2024-09-24'] }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Anthropic.NotFoundError);
  });
});
