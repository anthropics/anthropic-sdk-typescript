...
export class BetaToolRunner<Stream extends boolean> {
  ...
  #iterationCount = 0;
  /**
   * Container id most recently reported by the server, carried across iterations.
   *
   * Kept outside `#state.params` because `setMessagesParams()` replaces the params object wholesale,
  ...
  /**
   * Record the container the server used for this turn, so the next request can reuse it.
   *
   * Must run on every iteration — before the compaction check, and whether or not the consumer took over
   * the history — because both of those paths skip the assistant-turn `await` below, and a container
   * minted on such a turn still has to be forwarded. Without it the next request fails with
   * "container_id is required when there are pending tool uses generated by code execution with tools".
   *
   * A response reporting no container leaves a captured id alone: a turn that runs no code reports
   * `container: null` while the container it created is still alive.
   */
  async #captureContainer(): Promise<void> {
    try {
      ...
      // Reading `container` must not decide when a failed request surfaces. The awaits below and after
      // the loop own that.
    }
  }
    ...
    try {
      while (true) {
        ...
        try {
          ...
          // Snapshotted before the request so that history the consumer takes over during the yield is
          // detected afterwards, however they hand it to us. A copy, not the array itself: mutators are
          // free to edit `params.messages` in place, which a reference to the live array cannot see.
          const messagesAtRequest = [...this.#state.params.messages];

          if (this.#containerId !== undefined) {
            params.container = withContainerId(params.container, this.#containerId);
          }
          ...
          // The consumer owns the history from here if any message was added, moved, or swapped out during
          // the yield; a change to any other param (max_tokens, model, ...) leaves it ours to record.
          // Element identity rather than the array itself, because a mutator is free to edit in place.
          const messagesNow = this.#state.params.messages;
          const messagesReplaced =
            messagesNow.length !== messagesAtRequest.length ||
            messagesAtRequest.some((message, index) => messagesNow[index] !== message);
   ...
   *
   * Changing `messages` — replacing the array, or editing it in place — hands the conversation history to
   * the caller for that turn: the runner will not append the assistant turn it just received, since the
   * caller has already decided what the history should be. Changing any other parameter leaves the
   * history to the runner.
   *
...
/**
 * Fill in the server-reported container id, leaving a container the caller chose themselves alone.
 *
 * A caller's own `container` always wins: an id of their own, or an explicit `null` meaning they want no
 * container, is theirs to decide. Only what they left unset is filled in. `container` accepts an object
 * as well as an id, so a caller may have configured `{ skills: [...] }` with no id — a bare id would
 * discard that configuration, so the object is preserved and a copy is returned rather than mutating it.
 */
function withContainerId(
  container: BetaContainerParams | string | null | undefined,
  id: string,
): BetaContainerParams | string | null {
  if (container === undefined) {
    return id;
  }
  if (container === null || typeof container === 'string' || container.id != null) {
    return container;
  }
  return { ...container, id };
}
