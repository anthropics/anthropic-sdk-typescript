...
export class ToolsBetaMessageStream implements AsyncIterable<ToolsBetaMessageStreamEvent> {
   ...
   */
  async finalMessage(): Promise<ToolsBetaMessage> {
    await this.done();
  ...
  protected async _fromReadableStream(
    ...
    this.#beginRequest();
    this._connected();
    const stream = Stream.fromReadableStream<ToolsBetaMessageStreamEvent>(readableStream, this.controller);
    for await (const event of stream) {
      this.#addStreamEvent(event);
    }
    if (stream.controller.signal?.aborted) {
  ...
  [Symbol.asyncIterator](): AsyncIterator<ToolsBetaMessageStreamEvent> {
    ...
    return {
      next: async (): Promise<IteratorResult<ToolsBetaMessageStreamEvent>> => {
        if (!pushQueue.length) {
          if (done) {
            return { value: undefined, done: true };
          }
          return new Promise<ToolsBetaMessageStreamEvent | undefined>((resolve, reject) =>
